...
def shuffle_frame(frame):
    ...
    for [x, y] in rps:
        ...
        temp = temp.append(frame.loc[indices], ignore_index=True, verify_integrity=True)
...
def split_frame(frame, split=0.8, shuffle=True, target="label", seed=42):
    ...
    for lbl in rps:
        ...
        # append rows into train and test
        train = train.append(
            frame.loc[train_indices], ignore_index=True, verify_integrity=True
        )

        test = test.append(
            frame.loc[test_indices], ignore_index=True, verify_integrity=True
        )

    # check final shapes
    # number of columns should be same in all three
    assert train.shape[1] == test.shape[1] == frame.shape[1], "error splitting raw data"
...
def at_refpoint(data_frame: DataFrame, xy_point: list) -> DataFrame:
    ...
    return data_frame.loc[
        (data_frame["x"] == xy_point[0]) & (data_frame["y"] == xy_point[1])
    ]
...
def set_samples_per_ref_pt(frame, num_samples):
    ...
    for (i, xy_point) in frame[["x", "y"]].drop_duplicates().iterrows():
        ...
        at_ref_df = frame.loc[(frame["x"] == xy_point[0]) & (frame["y"] == xy_point[1])]
...
def make_images(vectors, fmt="tf", force_shape=None):
    ...
    if req_pad != 0:
        # do padding here
        vectors = np.hstack((vectors, np.zeros((vectors.shape[0], req_pad))))
...
def import_model(
    ...
    """Save a model at a path with some meta data
    ...
    model_file : str
        name of model file
    target_location : str
        target location for this model
    Returns
    -------
    tuple
        model and its meta data is returned
    """
    model_path = os.path.join(target_location, model_file)
    logging.info(f"loading model from {model_path}")
    model = keras.models.load_model(model_path)

    logging.info(f"loading meta from {model_path}/model_info.json")
    meta = json.load(open(f"{model_path}/model_info.json", "r"))

    return model, meta


def export_model(model, meta, save_path, overwrite=False):
    """export model to this location or default location at init

    :param model: keras model
    :param meta: dict to be saved as json
    :param save_path: path to model
    :param overwrite: overwrite model if already exists
    :returns:
    :rtype:
    """
